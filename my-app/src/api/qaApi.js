...
export function useQaApi() {
  ...
  const listQuestions = (params = {}) => {
    ...
    return req(`/api/qa/questions${qs ? `?${qs}` : ""}`);
  };
  const getQuestion = (id) => req(`/api/qa/questions/${id}`);
  ...
  return { listQuestions, getQuestion, askQuestion, voteQuestion, addAnswer, voteAnswer, acceptAnswer };
}
